<template>
  <v-app>
    <v-app-bar app color="cyan darken-4" dark>
      ...
      <v-spacer></v-spacer>
      <v-list dense color="cyan darken-4" dark>
        <v-list-item  link :to="items[0].to">
          Home
        </v-list-item>
      </v-list>
      <v-list dense color="cyan darken-4" dark>
        <v-list-item link :to="items[1].to">
    ...
    <v-navigation-drawer app v-model="sidebar" :mini-variant.sync="mini">
      ...
      <v-list-item class="px=2" @click="mini = !mini">
        ...
        <v-list-item-content> {{userundefined}} {{nome}} {{sobrenome}}</v-list-item-content>
...
export default {
  data() {
    return {
      ...
      sobrenome: "",
      uid: "",
      userundefined: "Usuário ainda não definido",
      temPerfil: false,
      sidebar: true,
      mini: false,
      items: [
